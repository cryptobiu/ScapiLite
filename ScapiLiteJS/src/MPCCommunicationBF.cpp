...
std::vector< std::shared_ptr<ProtocolPartyDataBF> > MPCCommunicationBF::setCommunication(int id, int numParties, std::string configFile)
{
    ...
    for (int i = 0; i < numParties; i++) {
    	uint16_t self_port = ports[id]+i, peer_port = ports[i]+id;
    	if(i < id)
    		peer_port -= 1;
    	else if (i>id)
    		self_port -= 1;
    	...
    	std::shared_ptr<CommPartyBF> channel = std::make_shared<CommPartyTCPSyncedBoostFree>(ips[id].c_str(), self_port, ips[i].c_str(), peer_port, id < i);
    	channel->join();
    	std::cout<<"channel is on the way to be established"<<std::endl;
