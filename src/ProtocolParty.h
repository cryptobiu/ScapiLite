...
void ProtocolParty<FieldType>::runOffline() {
    ...
    auto duration = duration_cast<milliseconds>(t2-t1).count();
    if(flag_print_timings) {
        cout << "time in milliseconds RandomSharingForInputs: " << duration << endl;
    }
    cout << "===================================" << '\n';
...
void ProtocolParty<FieldType>::roundFunctionSyncBroadcast(vector<byte> &message, vector<vector<byte>> &recBufs) {
    ...
    cout<<"Party ID: "<<parties[0]->getID()<<"; read "<<recBufs[parties[0]->getID()].size()<<"/"<<recBufs[parties[0]->getID()].size()<<" bytes"<<endl;
	cout<<"Party ID: "<<parties[1]->getID()<<"; read "<<recBufs[parties[1]->getID()].size()<<"/"<<recBufs[parties[1]->getID()].size()<<" bytes"<<endl;
}
